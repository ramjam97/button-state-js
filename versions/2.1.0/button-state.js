/*!
 * RamStateJs v2.1.0
 * Description: A lightweight utility for managing button states in vanilla JavaScript.
 * Author: Ram Jam
...
function useBtnState(btnSelector = null, options = {}) {
    ...
    const targetBtnList = [...getButtonElements()].map((btn) => {
        ...
        return {
            ...
            loadingInnerHTML,
            classes: {
                disabled: options?.classes?.disabled ?? 'disabled',
                loading: options?.classes?.loading ?? 'loading'
            }
        };
    });
